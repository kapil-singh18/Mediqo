    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <SectionCard title="Basic Personal Information" icon={<UserIcon className="w-4 h-4 text-[#5F6FFF]" />}>
          <div className="flex items-center gap-5 pb-4 border-b border-slate-100 mb-4">
            <DoctorAvatar
              src={profileImage}
              name={name || 'Doctor'}
              speciality={speciality || 'Medical Specialist'}
              className="w-16 h-16 rounded-full shrink-0"
            />

            <div>
              <h3 className="text-sm font-bold text-slate-900">{name || 'Doctor Name'}</h3>
              <p className="text-xs text-slate-500 font-medium">{speciality || 'Medical Specialist'}</p>
              <p className="text-[11px] text-[#5F6FFF] font-medium mt-0.5">Verified Medical Practitioner</p>
            </div>
          </div>
