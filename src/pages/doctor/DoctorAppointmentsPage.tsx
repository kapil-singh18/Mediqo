  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitle="View scheduled consultations, filter patient lists, and manage clinical statuses."
        badgeText="Patient Consultation Management"
        action={
          <Button
            variant="outline"
            size="sm"
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {appointments.map((apt) => (
            <Card
              key={apt._id}
              className="p-6 border border-slate-200 hover:border-[#5F6FFF]/40 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6"
            >
                ...
                {apt.status === 'scheduled' && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex-1 px-3 py-1.5 rounded-[8px] text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors flex items-center justify-center gap-1"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" /> Complete
                    </button>
                    <button
                      onClick={() => handleStatusChange(apt._id, 'cancelled')}
                      className="flex-1 px-3 py-1.5 rounded-[8px] text-xs font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 transition-colors flex items-center justify-center gap-1"
                    >
