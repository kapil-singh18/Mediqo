...
import { Stethoscope, LogOut, ChevronDown, Calendar, Users, Activity, Home } from 'lucide-react';
          ...
          <div className="flex items-center justify-between h-18">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold tracking-tight text-gray-900">
                Mediqo <span className="text-xs font-normal text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full ml-1">Doctor Portal</span>
              </span>
            </Link>
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-emerald-600 text-white text-xs font-bold flex items-center justify-center uppercase">
                  {user?.name?.charAt(0) || 'D'}
                </div>
                <div className="text-left hidden sm:block">
                  <p className="text-xs font-semibold text-gray-800">{user?.name}</p>
                  <p className="text-[10px] text-emerald-600 font-medium">{user?.speciality || 'Practitioner'}</p>
                </div>
